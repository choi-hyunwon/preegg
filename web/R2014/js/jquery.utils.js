	...
	$.fn.tileGallery = function (options) {
		...
		return this.each(function(i, v) {
			...
			if(totalImagesCount === 0) {
				//app.log('data length is 0. morebutton will be hide and no images text will be appear.');
			...
			*/
			function onLoadMoreItemEventHandler(e) {
				...
				if(maxItemCount >= totalImagesCount) {
					...
					$moreButton.hide(); // hide button when it doesn't necessary. ex.) total images count is under 20.
				}
			...
			*
			* @param {Image()} _this : images object has a source, index
			...
			function addToLoadedItems(_this, error) {
				...
				if(error) {
					if(config.type === 'event') {
						errorImage = { "url": window._NO_IMAGE.landscape, "link":"#", "width":config.imageWidth, "height":150, "alt":"default error images" };
					} else {
						errorImage = { "url": window._NO_IMAGE.square, "link":"#", "width":config.imageWidth, "height":config.imageWidth, "alt":"default error images" };
					}
			...
			* start AddItemEvent after every images loaded complete.
			*/
			...
			/*
			* add images to list event. triggered after load images.
			* @type : EVENT
			*/
			...
			*/
			function setContainerHeight() {
				// event has a different bottom margin because of images that right bottom corner in background
				if(config.type === 'event') {
