...
function fnFanMovieType(movieIdx, mGroupCD, mTitle) {		
        $.ajax({
            ...
            success: function (data) {         
					...
					var qUrl  ="http://m.cgv.co.kr/quickReservation/Default.aspx?MovieIdx=" + movieIdx;				
					...
					if(layerShowType)
					{
						...
						if (cgv.common.StandardInfo.IsWebView) {
							...
							var appUrl = 'http://m.cgv.co.kr/quickReservation/Default.aspx?MovieIdx=' + movieIdx + '&sm_type=';
							openNewWebViewV5(appUrl,'','H-37','', mGroupCD , mTitle,'00');
						}
						else
						{
							fnSendGALog('1', '', 'MW_홈', '팬페이지_지금예매', '');
							window.location.href="http://m.cgv.co.kr/quickReservation/Default.aspx?MovieIdx=" + movieIdx + "&sm_type=";
						}
					}			
