...
function fnTabMenu(tk){
	...
	// 관련소식
	if(tk == "310" || tk == "311" || tk == "1" || tk == "202" || tk == "204") {
	//if( tk == "1") {	
		/*if(tk == "310" || tk == "311"){
			if(strChartYN == "Y"){
				initLineChart();
	...
	// 실관람평
	} else if (tk == "201" || tk == "2" || tk == "205" || tk == "312" || tk == "313") {
	//} else if (tk == "2") {
		...
		$.ajax({
			...
			success: function(result){
               ...
               // if(cmtsPageStart == 1){
                	fnMovieCommentsList(sTotCnt, pageStart, '0', '0');
				//}
            	if(sTotCnt > pageSize){
	                $(window).off("scroll").on("scroll", function(e) {
				        if (jQuery(window).scrollTop() >= (jQuery(document).height() - jQuery(window).height()) * 0.7) {
				            ...
				            //	$("#fpTabCont2").find("div.cover_list > ul").append("<li><div class='con_loading'><img src='../images/common/loading2.gif' alt='페이지 불러오는 중' /></div></li>");
			            	 ...
			            	 if (isPreviousContentsComplete) {
				            	...
				            	$("#fpTabCont2").find("div.cover_list > ul").append("<li><div class='con_loading'><img src='../images/common/loading2.gif' alt='페이지 불러오는 중' /></div></li>");
			            		var orderType = $('.btn_filter.type1').attr('order-src');
				            	var filterType = $('.btn_sort_area').attr("filter-src");
				            	fnMovieCommentsList(sTotCnt, pageStart, orderType, filterType);
			            	 }
			            	 if(isListComplete) {
				                //isListComplete = false;
				                //alert("마지막 페이지 입니다.");
	...
	// 영화정보
	} else if (tk == "200" || tk == "3" || tk == "203" || tk == "314" || tk == "315") {
	//} else if (tk == "200" || tk == "201" || tk == "310" || tk == "311" | tk == "3") {	
		$.ajax({
			method: "post",
...
function fnMoveBiLink(agencyName, url){
	...
	var url = "http://moviestory.cgv.co.kr/fanpage/agencyView?agencyCd=" + url;
...
// 실관람평 목록 조회
function fnMovieCommentsList(dataTotCnt, pageStart, orderType, filterType){  
	...
	$.ajax({
		...
		dataType: "html",
		async: false,
		data: {movieIdx:strMovieIdx, pageStart:pageStart, pageSize:pageSize, orderType:orderType, filterType:filterType, maxCmtIdx:maxCmtIdx, chkYn:chkYn},
		success: function(result){
			...
			var btm = $('.float_btnbox').css("bottom");
			if(btm != '0px'){
				$('.float_btnbox').animate({"bottom":"0"}, 500);
				$('.float_btnbox').eq(0).css("z-index", "10000");
			}
		},
		error: function(){
				...
				if(IsView == "N") {
					...
					if(IsWebView_Master){
						if(gubun == 'C'){			//실관람평 등록
							...
							selectedReplyText = selectedReplyText.replace("&amp;", '&');
							selectedReplyText = selectedReplyText.replace("&lt;", '<');
							selectedReplyText = selectedReplyText.replace("&gt;", '>');
...
/* 실관람평 UI개선건 관련 */
function movieCmtsListSort(orderType, filterType){
	pageStart = 1;
	$('.customer_con').remove();
	var sTotCnt = $("#sAllCnt").val();
	fnMovieCommentsList(sTotCnt, pageStart, orderType, filterType);
